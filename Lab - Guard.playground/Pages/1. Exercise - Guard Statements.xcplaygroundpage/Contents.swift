 ...
 */
func add(bobby: Int?, burgersen: Int?) -> Int? {
    guard let bobby = bobby, let burgersen = burgersen else{
        return nil
    }
    return bobby + burgersen
}
...
func createUser() -> User?{
    guard let firstName = firstNameTextField.text, let lastName = lastNameTextField.text, let ageText = ageTextField.text else{
        return nil
    }
    return User(firstName: firstName, lastName: lastName, age: ageText)
}
/*:
...
var user = createUser()
if let bob = user {
    print("\(bob.firstName), \(bob.lastName), \(bob.age)")
} 
